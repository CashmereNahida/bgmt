...
#from .__init__ import patch_notes_message
patch_notes_message = None
class ShowPatchNotes(bpy.types.Operator):
    bl_idname = "view3d.show_patch_notes"
    bl_label = "패치노트 확인"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        # 패치노트 메시지를 줄바꿈 기준으로 나눕니다.
        message_lines = patch_notes_message.split('\n')
        
        # 패치노트를 팝업 메뉴로 그리는 함수입니다.
        def draw_patch_notes(self, context):
            for line in message_lines:
                self.layout.label(text=line)
        
        # 팝업 메뉴를 생성하고, 패치노트를 그리는 함수를 전달합니다.
        bpy.context.window_manager.popup_menu(draw_patch_notes, title="패치노트", icon='INFO')
...
class AddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    bl_label = "Update Genshin Mod Tools".format(x=updater.addon)
...
class AddonUpdaterCheckNow(bpy.types.Operator):
    ...
    bl_description = "Check now for an update to the Genshin Mod Tools".format(
        updater.addon)
    bl_options = {'REGISTER', 'INTERNAL'}
...
class AddonUpdaterUpdateNow(bpy.types.Operator):
    ...
    bl_description = "Update to the latest version of the Genshin Mod Tools".format(
        x=updater.addon)
    bl_options = {'REGISTER', 'INTERNAL'}
...
class AddonUpdaterUpdateTarget(bpy.types.Operator):
    ...
    bl_description = "Install a targeted version of the Genshin Mod Tools".format(
        x=updater.addon)
    bl_options = {'REGISTER', 'INTERNAL'}
...
def update_settings_ui(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        split.operator(AddonUpdaterUpdateNow.bl_idname,
                       text=str(updater.update_version) + " 로 업데이트")
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
    ...
    if not updater.manual_only:
        ...
        if updater.include_branches and len(updater.include_branch_list) > 0:
            ...
            col.operator(AddonUpdaterUpdateTarget.bl_idname,
                         text="애드온 재설치")
            #col.operator("view3d.show_patch_notes", text="패치노트 확인")
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        split.enabled = False
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname, text="확인중...")
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(AddonUpdaterEndBackground.bl_idname, text="", icon="X")
...
def register(bl_info):
    ...
    updater.website = "https://github.com/CashmereNahida/bgmt/archive/master.zip"
